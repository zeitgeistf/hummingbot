...
cdef class LiquidMarket(MarketBase):
    ...
    def __init__(self,
        ...
        self._liquid_auth = LiquidAuth(liquid_api_key, liquid_secret_key, liquid_passphrase)
        self._order_book_tracker = LiquidOrderBookTracker(
            data_source_type=order_book_tracker_data_source_type, symbols=symbols)
        self._user_stream_tracker = LiquidUserStreamTracker(liquid_auth=self._liquid_auth, symbols=symbols)
    ...
    async def _api_request(self,
        ...
        url = f"{Constants.BASE_URL}{path_url}" if url is None else url  # FIXME
    ...
    async def list_liquid_accounts(self) -> Dict[str, str]:
        ...
        :returns: json response
        """
        path_url = "/liquid-accounts"
        liquid_accounts = await self._api_request("get", path_url=path_url)
        ids = [a["id"] for a in liquid_accounts]
        currencies = [a["currency"] for a in liquid_accounts]
    ...
    async def get_deposit_address(self, asset: str) -> str:
        ...
        :returns: json response
        """
        liquid_account_id_dict = await self.list_liquid_accounts()
        account_id = liquid_account_id_dict.get(asset)
        path_url = f"/liquid-accounts/{account_id}/addresses"
